...
/* ═══════════════════════════════════════════════════════════════════
   RAPPIBELLION T2E BRIDGE PATCH v2.0
   Cubre los 4 gaps vs Pac-Hack: player field, GAME_OVER, overlay.
   Este bloque replica lo que se appendea en script_min.js.
   ═══════════════════════════════════════════════════════════════════ */
(function() {
	'use strict';
	...
	console.info('[Tetris T2E Patch] Activo como', PLAYER.toUpperCase());

	// ── 2. WRAP postMessage → inyectar player en todos los mensajes ──
	var _origPostMessage = window.parent.postMessage.bind(window.parent);
	window.parent.postMessage = function(data, origin) {
		if (data && typeof data === 'object' && data.type) {
			var bridgeMsgs = ['SCORE_UPDATE','BOOST_USED','GAME_OVER','PLAY_AUDIO'];
			if (bridgeMsgs.indexOf(data.type) !== -1 && !data.player) {
				data = Object.assign({}, data, { player: PLAYER });
			}
		}
		_origPostMessage(data, origin || '*');
	};

	// ── 3. GAME_OVER cuando el tablero se llena ───────────────────────
	var _gameoverSent = false;
	var _lastGameState = null;
	setInterval(function() {
		var m = window.main;
		if (!m) return;
		var isGameOver = m.currentGameState === m.stateOver;
		if (isGameOver && !_gameoverSent) {
			_gameoverSent = true;
			var score = (m.statePlaying && m.statePlaying.stats)
				? (m.statePlaying.stats.score || 0) : 0;
			window.parent.postMessage({ type: 'GAME_OVER', score: score }, '*');
			console.info('[Tetris T2E Patch] GAME_OVER emitido. Score:', score);
		}
		if (!isGameOver && m.currentGameState === m.statePlaying) {
			_gameoverSent = false;
		}
	}, 200);

	// ── 4. Overlay VICTORIA/DERROTA en MATCH_ENDED ───────────────────
	var _matchEnded = false;
	window.addEventListener('message', function(ev) {
		var d = ev.data;
		if (!d || d.type !== 'MATCH_ENDED' || _matchEnded) return;
		_matchEnded = true;
		setTimeout(function() { _showT2EOverlay(d.winner); }, 400);
	});

	function _showT2EOverlay(winner) {
		var isWinner = winner === PLAYER;
		var isDraw   = winner === 'draw';
		var color    = isDraw ? '#FFD700' : isWinner ? '#00BCFF' : '#FF2D78';
		var title    = isDraw ? '⚡ EMPATE' : isWinner ? '▶ VICTORIA' : '✕ DERROTA';
		var score    = (window.main && window.main.statePlaying && window.main.statePlaying.stats)
			? (window.main.statePlaying.stats.score || 0) : 0;

		var style = document.createElement('style');
		style.textContent = '@keyframes t2eFadeIn{from{opacity:0}to{opacity:1}}'
			+ '@keyframes t2eGlow{0%,100%{text-shadow:0 0 20px currentColor}'
		...
		overlay.style.cssText = 'position:fixed;inset:0;z-index:2147483647;'
			...
			+ 'background:rgba(2,4,8,0.90);font-family:Orbitron,Courier New,monospace;'
			+ 'animation:t2eFadeIn 0.4s ease;';
		overlay.innerHTML = '<div style="font-size:clamp(28px,7vw,58px);font-weight:900;color:' + color
			+ ';letter-spacing:6px;animation:t2eGlow 1.5s ease-in-out infinite;margin-bottom:14px;">'
			+ title + '</div>'
			+ '<div style="color:rgba(0,255,65,0.55);font-size:13px;letter-spacing:4px;'
			+ 'font-family:\'Share Tech Mono\',monospace;">SCORE FINAL // '
			+ String(score).padStart(6,'0') + '</div>'
			+ '<div style="margin-top:18px;color:rgba(0,188,255,0.35);font-size:10px;'
			+ 'letter-spacing:5px;font-family:\'Share Tech Mono\',monospace;">'
			+ 'RAPPIBELLION // TETRIS-CIPHER T2E</div>';
