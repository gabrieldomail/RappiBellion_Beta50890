...
function updateTime(elapsedTime, stats, fighter) {
    ...
    // --- GESTIÓN DE BOOST (Para el brillo dorado del Fighter) ---
    if (window.MyGameFX && MyGameFX.boostActive > 0) {
        MyGameFX.boostActive -= elapsedTime;
        if (MyGameFX.boostActive <= 0) MyGameFX.boostActive = 0;
    }
        ...
        for (let i = 0; i < stageEnemies.length; i++) {
            if (stageEnemies[i].time < stats.stage.stageTime) {
                for (let j = 0; j < stageEnemies[i].enemy.length; j++) {
                    let e = stageEnemies[i].enemy[j];
                    
                    // --- ASIGNACIÓN DE VIDA MÁXIMA (Para la Barra de Vida) ---
                    // Si el enemigo no tiene maxHp, lo asignamos ahora para que render.js pueda dibujarlo
                    if (e.hp !== undefined && e.maxHp === undefined) {
                        e.maxHp = e.hp;
                    }
                    
                    enemies.enemy.push(e);
                }
                remove.push(i);
            }
        }
...
function updateEnemy(elapsedTime, enemy, stats, enemies, torpedos, fighter) {
    // Aseguramos que maxHp exista para la barra de vida
...
function updateEnemyFormation(elapsedTime, enemies) {
    ...
    if (enemies.formationLeftRight > 0) {
        if (enemies.formationLeftRight % 2 === 0) enemies.formationOffsetX += speed;
        else enemies.formationOffsetX -= speed;
        if (enemies.formationOffsetX > 100) { enemies.formationOffsetX = 100; enemies.formationLeftRight -= 1; }
        else if (enemies.formationOffsetX < -100) { enemies.formationOffsetX = -100; enemies.formationLeftRight -= 1; }
        else if (enemies.formationLeftRight === 1 && enemies.formationOffsetX < 0) { enemies.formationOffsetX = 0; enemies.formationLeftRight = 0; }
    } else if (enemies.formationLeftRight === 0) {
...
function enemyDiving(elapsedTime, enemies, stats, sound) {
    ...
    if (enemies.divingTimer < 0) {
        ...
        for (let i = 0; i < enemies.enemy.length; i++) {
            ...
            if (!enemy.diving && enemy.path.length === 0) availableEnemies.push(enemy);
        }
...
function updateTorpedos(elapsedTime, torpedos) {
    ...
    for (let i = removeFriendly.length-1; i > -1; i--) torpedos.friendly.splice(removeFriendly[i], 1);
    let removeEnemy = [];
    ...
    for (let i = removeEnemy.length-1; i > -1; i--) torpedos.enemy.splice(removeEnemy[i], 1);
}
...
function moveFighterLeft(fighter, value) {
    ...
    if (fighter.center.x < fighter.size.width/2) fighter.center.x = fighter.size.width/2;
}

function moveFighterRight(fighter, value) {
    let canvas = document.getElementById('id-canvas');
    fighter.center.x += 8 * value;
    if (fighter.center.x + fighter.size.width/2 > canvas.width) fighter.center.x = canvas.width - fighter.size.width/2;
}
...
function checkFighterCollision(torpedos, fighter, enemies, particles, sound) {
    ...
    for (let i = removeTorpedo.length-1; i > -1; i--) enemyTorpedos.splice(removeTorpedo[i], 1);
    ...
    for (let i = 0; i < enemies.enemy.length; i++) {
        ...
        if (e.center.x < fighter.center.x+fighter.size.width/2+20 && e.center.x > fighter.center.x-fighter.size.width/2-20 &&
            ...
            createPlayerDeathParticles(fighter, particles);
            loseLife(fighter);
            createEnemyDeathParticles(particles, enemies, e); // <--- DISPARA POPUP DE PUNTOS
            if (sound.playerDeath.isReady && !attractMode) sound.playerDeath.play();
            removeEnemy.push(i);
        }
    }
    for (let i = removeEnemy.length-1; i > -1; i--) enemies.enemy.splice(removeEnemy[i], 1);
}
...
function checkEnemyCollision(torpedos, enemies, stats, particles, sound) {
            ...
            if (t.center.x < e.center.x+enemies[e.type].size.width/2+6 && t.center.x > e.center.x-enemies[e.type].size.width/2-6 && 
                ...
                if (!removeEnemy.includes(j)) {
                    if (e.type === "boss" && e.life > 1) {
                        ...
                        addScore(stats, e);
                        createEnemyDeathParticles(particles, enemies, e); // <--- DISPARA POPUP DE PUNTOS
                        if (e.type === "boss" && sound.bossDeath.isReady && !attractMode) sound.bossDeath.play();
                        else if (sound.enemyDeath.isReady && !attractMode) sound.enemyDeath.play();
                    }
                }
                stats.stage.hits++;
            }
    ...
    for (let i = removeTorpedo.length-1; i > -1; i--) friendlyTorpedos.splice(removeTorpedo[i], 1);
    for (let i = removeEnemy.length-1; i > -1; i--) enemies.enemy.splice(removeEnemy[i], 1);
}
...
function addScore(stats, enemy) {
    if (enemy.type === "bee" && !enemy.diving) stats.score += 50;
    else if (enemy.type === "bee" && enemy.diving) stats.score += 100;
    else if (enemy.type === "butterfly" && !enemy.diving) stats.score += 80;
    else if (enemy.type === "butterfly" && enemy.diving) stats.score += 160;
    else if (enemy.type === "bonus1" || enemy.type === "bonus2" || enemy.type === "bonus3") stats.score += 100;
    else if (enemy.type === "boss" && !enemy.diving) stats.score += 150;
    else if (enemy.type === "boss" && enemy.diving) stats.score += 400;
}
...
function mobileSupport(fighter, torpedos, stats, sound) {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera MiniS/i.test(navigator.userAgent)) {
        ...
        var start;
        slider.addEventListener("touchstart", function(event) { start = event.touches[0].clientX; });
        ...
        slider.addEventListener("input", function() { fighter.mobileMoveVal = slider.value; });
        slider.addEventListener("touchend", function(event) { slider.value = 50; fighter.mobileMoveVal = 50; });
