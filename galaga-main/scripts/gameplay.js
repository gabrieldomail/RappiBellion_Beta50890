MyGame.game = (function(game, input, graphics, images, sounds) {
    'use strict';
    ...
    function update(elapsedTime) {
        ...
        if (typeof attractMode !== 'undefined' && attractMode) {
            updateAI(elapsedTime, ai, fighter, enemies, torpedos, stats, sound);
            if (stats.stage && stats.stage.currentStage === 3) {
                endGame();
            }
        }
        checkCollisions(torpedos, fighter, enemies, stats, particles, sound);
        
        // 1. Actualiza las partículas comunes (humo, fuego)
        updateParticles(particles.particle, elapsedTime);
        
        // 2. === AÑADE ESTA LÍNEA AQUÍ ===
        // Esto hace que los popups suban, desaparezcan y que el shake se detenga
        if (typeof updateGlobalFX === 'function') {
            updateGlobalFX(elapsedTime);
        }
        // ==============================

        checkEndStage(enemies, stats, fighter, elapsedTime, sound);
        
        if (fighter && fighter.lives === 0 && stats.endGameTimer <= 0) {
            endGame();
        }
    }
    ...
    function render() {
        ...
        // === ESTAS DOS LÍNEAS ACTIVAN LOS EFECTOS NUEVOS ===
        graphics.drawPopups(); 
        graphics.drawShockwaves(); 
        // =================================================
    ...
    function run() {
        ...
        function endGame() {
        cancelNextRequest = true; // Detiene el loop la primera vez
        ...
        if (typeof attractMode !== 'undefined' && attractMode) {
            ...
            attractMode = false;
            MyGame.screens.showScreen('main-menu'); // Usa el nuevo sistema
        } else if (quit) {
            MyGame.screens.showScreen('main-menu');
        } else {
            saveScoreValue(stats.score);
            MyGame.screens.showScreen('high-scores');
        }
        resetGame();
    }
