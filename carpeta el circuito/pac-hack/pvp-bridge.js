...
(function() {
  ...
  function applyPvpScale() {
    ...
    function scale() {
        ...
        // No escalar si el iframe tiene dimensiones 0 (está oculto)
        var w = window.innerWidth;
        var h = window.innerHeight;
        if (w === 0 || h === 0) {
            // Reintentar cuando el iframe tenga dimensiones reales
            setTimeout(scale, 200);
            return;
        }

        // Scale para llenar el iframe SIN exceder 1:1 en free play
        // En PvP (pantalla partida) puede subir más, en free play no queremos distorsión
        var scaleRaw = Math.min(w / GAME_W, h / GAME_H);
        var s = Math.min(scaleRaw, 1.8); // máximo 1.8x para evitar desborde
        ...
        // Asegurar que el body del iframe contenga el juego escalado
        document.body.style.overflow = 'hidden';
        document.body.style.position = 'relative';
        document.documentElement.style.overflow = 'hidden';

        console.info('[PvP Bridge] Scale aplicado: ' + s.toFixed(3) +
            ' (' + Math.round(w) + 'x' + Math.round(h) + ' raw:' + scaleRaw.toFixed(3) + ')');
    }

    // Esperar container + que el iframe tenga dimensiones reales
    var attempts = 0;
    var waitForContainer = setInterval(function() {
        attempts++;
        var container = document.getElementById('container');
        var hasSize   = window.innerWidth > 0 && window.innerHeight > 0;
        if ((container && hasSize) || attempts > 40) {
