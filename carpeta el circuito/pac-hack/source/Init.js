(function () {
    ...
    function requestAnimation() {
        ...
        animation = window.requestAnimationFrame(() => {
            ...
            // timer iniciado en newGame()
            const _elapsed    = (Date.now() - (window._hackerSessionStart || Date.now())) / 1000;
            const _offsetSecs = 240;   // arranca en equivalente al minuto 4
            const _minDiv     = 5;     // infierno puro
            const _maxDiv     = 16;    // velocidad normal
            const _rampSecs   = 480;
            ...
            const _speedCap   = 5 + (_curve * 6);

            if (speed > _speedCap) {
                return requestAnimation();
            }

            // Notificar nivel de caos al HUD padre
            // Actualizar chaos level cada ~2 segundos (no en cada frame)
            if (window.parent && window.parent !== window) {
                var _now = Date.now();
                if (!window._lastChaosUpdate || _now - window._lastChaosUpdate > 2000) {
                    window._lastChaosUpdate = _now;
                    var _level = Math.round(_curve * 100);
                    if (!isNaN(_level)) {
     ...
     */
    function newGame() {
        // _hackerSessionStart es el inicio de SESIÓN — nunca se resetea
        // La velocidad/caos es continua entre partidas
        if (!window._hackerSessionStart) window._hackerSessionStart = Date.now();
        ...
        if (pvpPlayer === "p1" || pvpPlayer === "p2") {
            ...
            window.addEventListener("message", function(e) {
                ...
                if (d.type === "MATCH_ENDED") {
                    // Solo pausar en modo PvP real (p1/p2)
                    // En free play el padre maneja el veredicto — no interrumpir el juego
                    if (pvpPlayer && display.isPlaying()) {
                        display.set("paused");
                        animations.paused();
                    }
                }
